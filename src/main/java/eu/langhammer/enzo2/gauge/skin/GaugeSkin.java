 ...
 */
package eu.langhammer.enzo2.gauge.skin;

import eu.langhammer.enzo2.common.Marker;
import eu.langhammer.enzo2.common.Section;
import eu.langhammer.enzo2.common.ValueEvent;
import eu.langhammer.enzo2.gauge.Gauge;
...
/**
 * Created by User: hansolo Date: 01.04.13 Time: 17:18
 */
public class GaugeSkin extends SkinBase<Gauge> implements Skin<Gauge> {
    ...
    protected void handleControlPropertyChanged( final String PROPERTY ) {
        if ( "RESIZE".equals( PROPERTY ) ) {
            ...
            else if ( currentValue > getSkinnable().getThreshold() ) {
            ...
            for ( Marker marker : getSkinnable().getMarkers().keySet() ) {
                if ( marker.isExceeded() ) {
                    if ( currentValue < marker.getValue() ) {
                        marker.fireMarkerEvent( new ValueEvent( this, null, ValueEvent.VALUE_UNDERRUN ) );
                        marker.setExceeded( false );
                    }
                }
                else if ( currentValue > marker.getValue() ) {
                    marker.fireMarkerEvent( new ValueEvent( this, null, ValueEvent.VALUE_EXCEEDED ) );
